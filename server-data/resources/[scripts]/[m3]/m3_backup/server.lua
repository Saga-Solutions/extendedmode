...
TriggerEvent('es:addGroupCommand', 'kod', 'user', function(source, args, user)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local level   = args[1]
	if GetCurrentResourceName() == 'm3_backup' then
		if xPlayer.job.name == 'police' or xPlayer.job.name == 'sheriff' then
			if level ~= nil and level == '1' or level == '2' or level == '3' then
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'error', text = 'Kodu boş bırakamazsınız! /kod 1 /kod 2 /kod 3'})
			end
		end
	else
