...
AddEventHandler('m3:gps:updateBlips', function()
	...
	if PlayerData.job.name == Config.Job1 or PlayerData.job.name == Config.Job2 or PlayerData.job.name == Config.Job3 then
...
function createBlip(id, firstname, lastname, code) -- police
	...
	if not DoesBlipExist(blip) then -- Add blip and create head display on player
		...
		AddTextComponentString('[~b~'.. code ..'~s~] - ' .. firstname .. ' ' .. lastname ..'')
...
function createBlip2(id, firstname, lastname, code) -- ambulance
	...
	if not DoesBlipExist(blip) then -- Add blip and create head display on player
		...
		SetBlipAsShortRange(blip, true)
		BeginTextCommandSetBlipName('STRING')
		AddTextComponentString('[~r~'.. code ..'~s~] - ' .. firstname .. ' ' .. lastname ..'')
		EndTextCommandSetBlipName(blip)
...
function createBlip3(id, firstname, lastname, code) -- sheriff
	...
	if not DoesBlipExist(blip) then -- Add blip and create head display on player
		...
		SetBlipAsShortRange(blip, true)
		BeginTextCommandSetBlipName('STRING')
		AddTextComponentString('[~y~'.. code ..'~s~] - ' .. firstname .. ' ' .. lastname ..'')
		EndTextCommandSetBlipName(blip)
...
function createBlip4(id, firstname, lastname, code)
	local ped = GetPlayerPed(id)
	local blip = GetBlipFromEntity(ped)
...
function createBlip5(id, firstname, lastname, code)
	local ped = GetPlayerPed(id)
	local blip = GetBlipFromEntity(ped)
...
function createBlip6(id, firstname, lastname, code)
	local ped = GetPlayerPed(id)
	local blip = GetBlipFromEntity(ped)
