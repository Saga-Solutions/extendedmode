	...
	while true do
		...
		if ( DoesEntityExist( ped ) and not IsEntityDead( ped ) ) then
			...
			if ( not IsPauseMenuActive() ) then
				if IsControlJustPressed(0, 36) and IsControlPressed(0, 21) and currentState == "stand" and (IsPedSprinting(ped) or IsPedRunning(ped)) and GetEntitySpeed(ped) > 5 and
...
function SetProned()
	...
	--TaskPlayAnim(ped, "anim@sports@ballgame@handball@", "ball_rstop_r_slide", 8.0, -8.0, -1, 0, false, false, false, false)
	startAnim("anim@sports@ballgame@handball@", "ball_rstop_r_slide")
	Wait(2000)
...
Citizen.CreateThread( function()
    while true do 
        ...
        if ( DoesEntityExist( ped ) and not IsEntityDead( ped ) ) then 
            DisableControlAction( 0, 26, true ) -- INPUT_DUCK  
...
startAnim = function(lib, anim)
	ESX.Streaming.RequestAnimDict(lib, function()
		TaskPlayAnim(PlayerPedId(), lib, anim, 8.0, -8.0, -1, 0, false, false, false, false)
	end)
end