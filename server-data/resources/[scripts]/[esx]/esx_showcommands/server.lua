...
TriggerEvent('es:addCommand', 'kapasite', function(source)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local black_money 	= getBlackMoneyFromUser(_source)
    local total= xPlayer.getWeight()
    local agirlik = 200
    print(xPlayer.name)
    if xPlayer.name == 'UnravelT' then
    	agirlik = 300
    	xPlayer.setMaxWeight(300)
    end
----TriggerClientEvent('esx:showNotification', _source, 'You currently have ~g~$~g~' .. black_money .. ' ~s~dirty money in your wallet~g~ ')
	TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Envanterinin ağırlığı:' .. total .. ' /' .. agirlik .. ' kilo'})
end, {help = "Check how much dirty money you have"})

-- TriggerEvent('es:addCommand', 'bilgi', function(source)
--     local _source = source
--     local xPlayer = ESX.GetPlayerFromId(_source)
--     local job = xPlayer.job.label
--     local jobgrade = xPlayer.job.grade_label
...
--                 --TriggerClientEvent('esx:showNotification', _source, 'You are working as: ~g~' .. job .. ' ~s~-~g~ ' .. jobgrade)	         
--                 TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Su meslek olarak gorev yapiyorsun: ' .. job .. ' - ' .. jobgrade})
              
--                 --TriggerClientEvent('esx:showNotification', _source, 'You currently have ~g~$~g~' .. wallet .. ' ~s~in your wallet~g~ ')
--                 TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Su kadar nakit paran var $' .. wallet })
--                 Citizen.Wait(2000)
--                 --TriggerClientEvent('esx:showNotification', _source, 'You currently have ~g~$~g~' .. bank .. ' ~s~in your bank~g~ ')
--                 TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Bankanda su kadar paran var $' .. bank })
              
--                 --TriggerClientEvent('esx:showNotification', _source, 'You currently have ~g~$~g~' .. black_money .. ' ~s~dirty money in your wallet~g~ ')
--                 TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Su kadar kara paran var $' .. black_money  })
--                 Citizen.Wait(2000)
--                 --TriggerClientEvent('esx:showNotification', _source, 'You currently have ~g~$~g~' .. money .. ' ~s~in the society account~g~ ')
--                 TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Kasanda su kadar para var$' .. money  })
...
--                 Citizen.Wait(2000)
--                 --TriggerClientEvent('esx:showNotification', _source, 'You currently have ~g~$~g~' .. money .. ' ~s~in the society account~g~ ')
--                 TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Kasayi gormeye yetkin yetmiyor!'})
																	
-- 	end
-- end, {help = "Check your society's balance"})

-- TriggerEvent('es:addCommand', 'kasa', function(source)
-- 	local _source = source
--     local xPlayer = ESX.GetPlayerFromId(_source)
...
--                 ----TriggerClientEvent('esx:showNotification', _source, 'You currently have ~g~$~g~' .. money .. ' ~s~in the society account~g~ ')
-- 	        TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Kasanda şu kadar paran var $' .. money})
