...
RegisterNetEvent('ia-outlawalert:gunshotvehicleInProgress')
AddEventHandler('ia-outlawalert:gunshotvehicleInProgress', function(targetCoords)
	if Config.gunAlert then
		...
		AddTextComponentString('10-71 Ateş İhbari')              -- to 'supermarket'
...
RegisterNetEvent('ia-outlawalert:combatInProgress')
AddEventHandler('ia-outlawalert:combatInProgress', function(targetCoords)
...
AddEventHandler('police:tenThirteenA', function()
  if ESX.GetPlayerData().job.name == 'police' or ESX.GetPlayerData().job.name == 'ambulance' then	
		...
		TriggerEvent('ia-alerts:policealertA')
	end
end)

RegisterNetEvent('ia-alerts:policealertA')
AddEventHandler('ia-alerts:policealertA', function(targetCoords)
  if ESX.GetPlayerData().job.name == 'police' or ESX.GetPlayerData().job.name == 'ambulance' then	
		local alpha = 250
		local targetCoords = GetEntityCoords(PlayerPedId(), true)
		...
		AddTextComponentString('10-13A Yaralı Memur')
...
RegisterNetEvent('ambulance-death')
AddEventHandler('ambulance-death', function()
    if ESX.GetPlayerData().job.name == 'ambulance' then
		...
		TriggerEvent('ia-alerts:death')
	end
end)

RegisterNetEvent('ia-alerts:death')
AddEventHandler('ia-alerts:death', function(targetCoords)
    if ESX.GetPlayerData().job.name == 'ambulance' then
		local alpha = 250
		local targetCoords = GetEntityCoords(PlayerPedId(), true)
		local policedown = AddBlipForCoord(targetCoords.x, targetCoords.y, targetCoords.z)
		...
		SetBlipAsShortRange(policedown,  1)
		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString('311-14A Yaralı Ihbarı')
		EndTextCommandSetBlipName(policedown)
		TriggerServerEvent('InteractSound_SV:PlayOnSource', '10-1314', 0.3)
...
-- 			}
-- 		})
-- 		TriggerEvent('ia-alerts:policealertB')
-- 	end
-- end)

-- RegisterNetEvent('ia-alerts:policealertB')
-- AddEventHandler('ia-alerts:policealertB', function(targetCoords)
-- if ESX.GetPlayerData().job.name == 'police' or ESX.GetPlayerData().job.name == 'ambulance' then	
-- 		local alpha = 250
-- 		local targetCoords = GetEntityCoords(PlayerPedId(), true)
...
-- 			}
-- 		})
-- 		TriggerEvent('ia-alerts:panic')
-- 	end
-- end)

-- RegisterNetEvent('ia-alerts:panic')
-- AddEventHandler('ia-alerts:panic', function(targetCoords)
-- if ESX.GetPlayerData().job.name == 'police' or ESX.GetPlayerData().job.name == 'ambulance' then	
-- 		local alpha = 250
-- 		local targetCoords = GetEntityCoords(PlayerPedId(), true)
...
RegisterNetEvent("doctor:tenForteenB")
AddEventHandler("doctor:tenForteenB", function()
if ESX.GetPlayerData().job.name == 'police' or ESX.GetPlayerData().job.name == 'ambulance' then	
	local pos = GetEntityCoords(PlayerPedId(),  true)
	TriggerServerEvent("dispatch:svNotify", {
		dispatchCode = "10-14B",
...
RegisterNetEvent('ia-alerts:tenForteenB')
AddEventHandler('ia-alerts:tenForteenB', function(targetCoords)
if ESX.GetPlayerData().job.name == 'police' or ESX.GetPlayerData().job.name == 'ambulance' then	
		local alpha = 250
		local targetCoords = GetEntityCoords(PlayerPedId(), true)
		...
		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString('10-14B Yaralı Doktor')
...
-- 		}
-- 	})
-- 		TriggerEvent('ia-alerts:tenForteenA')
-- 	end
-- end)

-- RegisterNetEvent('ia-alerts:tenForteenA')
-- AddEventHandler('ia-alerts:tenForteenA', function(targetCoords)
--   if ESX.GetPlayerData().job.name == 'police' or ESX.GetPlayerData().job.name == 'ambulance' then	
-- 		local alpha = 250
-- 		local targetCoords = GetEntityCoords(PlayerPedId(), true)
...
-- RegisterNetEvent('ia-alerts:downalert')
-- AddEventHandler('ia-alerts:downalert', function(targetCoords)
-- if ESX.GetPlayerData().job.name == 'police' or ESX.GetPlayerData().job.name == 'ambulance' then	
-- 		local alpha = 250
-- 		local targetCoords = GetEntityCoords(PlayerPedId(), true)
...
---- Car Crash ----
RegisterNetEvent('ia-alerts:vehiclecrash')
AddEventHandler('ia-alerts:vehiclecrash', function(targetCoords)
if ESX.GetPlayerData().job.name == 'police' or ESX.GetPlayerData().job.name == 'ambulance' then	
		local alpha = 250
		local targetCoords = GetEntityCoords(PlayerPedId(), true)
		local injured = AddBlipForCoord(targetCoords.x, targetCoords.y, targetCoords.z)
...
RegisterNetEvent('ia-alerts:vehiclesteal')
AddEventHandler('ia-alerts:vehiclesteal', function(targetCoords)
if ESX.GetPlayerData().job.name == 'police' then	
		local alpha = 250
		local targetCoords = GetEntityCoords(PlayerPedId(), true)
		...
		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString('10-60 Çalıntı Araç')
		EndTextCommandSetBlipName(thiefBlip)
		TriggerServerEvent('InteractSound_SV:PlayOnSource', 'HighPrioCrime', 0.3)
...
RegisterNetEvent('ia-alerts:storerobbery')
AddEventHandler('ia-alerts:storerobbery', function(targetCoords)
if ESX.GetPlayerData().job.name == 'police' then	
		local alpha = 250
		local targetCoords = GetEntityCoords(PlayerPedId(), true)
		...
		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString('10-31B Market Soygunu')
...
RegisterNetEvent('ia-alerts:houserobbery')
AddEventHandler('ia-alerts:houserobbery', function(targetCoords)
if ESX.GetPlayerData().job.name == 'police' then	
		local alpha = 250
		local targetCoords = GetEntityCoords(PlayerPedId(), true)
		...
		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString('10-31A Ev Soygunu Olabilir')
...
RegisterNetEvent('ia-alerts:banktruck')
AddEventHandler('ia-alerts:banktruck', function(targetCoords)
	if ESX.GetPlayerData().job.name == 'police' then	
		local alpha = 250
		local targetCoords = GetEntityCoords(PlayerPedId(), true)
...
RegisterNetEvent('ia-alerts:jewelrobbey')
AddEventHandler('ia-alerts:jewelrobbey', function()
	if ESX.GetPlayerData().job.name == 'police' then	
		local alpha = 250
		local targetCoords = GetEntityCoords(PlayerPedId(), true)
		...
		SetBlipAsShortRange(Blip,  1)
		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString('10-90 Kuyumcu Soygunu')
...
RegisterNetEvent('ia-alerts:veznerobbery')
AddEventHandler('ia-alerts:veznerobbery', function()
	if ESX.GetPlayerData().job.name == 'police' then	
		local alpha = 250
		local targetCoords = GetEntityCoords(PlayerPedId(), true)
		local vezne = AddBlipForCoord(targetCoords.x, targetCoords.y, targetCoords.z)

		SetBlipSprite(vezne,  134)
		SetBlipColour(vezne,  0)
		SetBlipScale(vezne, 2.5)
		SetBlipAsShortRange(Blip,  1)
		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString('10-41P Vezne Soygunu')
		EndTextCommandSetBlipName(vezne)
		TriggerServerEvent('InteractSound_SV:PlayOnSource', 'HighPrioCrime', 0.3)

		while alpha ~= 0 do
			Citizen.Wait(120 * 4)
			alpha = alpha - 1
			SetBlipAlpha(vezne, alpha)
...
RegisterNetEvent('ia-alerts:jailbreak')
AddEventHandler('ia-alerts:jailbreak', function(targetCoords)
	if ESX.GetPlayerData().job.name == 'police' then	
		local alpha = 250
		local targetCoords = GetEntityCoords(PlayerPedId(), true)
