...
UTK = {
    ...
    alarmblip,
    text = { -- Texts
        loudstart = "[~r~E~w~] ~g~Alarmlı~w~ bir sekilde bankayı soy",
        silentstart = "[~r~E~w~] ~g~Sessiz~w~ bir sekilde bankayı soy",
        usecard = "[~r~E~w~] ID Card kullan",
        usethermal = "[~r~E~w~] Termal Charge kullan",
        usehack = "[~r~E~w~] Güvenlik panelini hackle",
        uselockpick = "[~r~E~w~] Maymuncuk kullan",
        usesearch = "[~r~E~w~] Ara",
        lootcash= "[~r~E~w~] Paraları çantana doldur",
        lootgold = "[~r~E~w~] Altınları çantana doldur",
        lootdia = "[~r~E~w~] Elmasları çantana doldur",
        card = "ID Card kullanılıyor",
        thermal = "Thermal Charge kuruluyor",
        burning = "Kilit eriyor",
        lockpick = "Kilit maymuncuklanıyor",
        using = "Panele erişiliyor",
        used = "Panele erişildi.",
        stage = "Tamamlandı.",
        search = "Aranıyor",
        hacking = "Hackleniyor",
        melted = "Kapı kilidi eridi.",
        hacked = "Hackleme başarılı.",
        unlocked = "Kapı kilidi açıldı.",
        nothing = "Bir şey bulamadın.",
        found = "ID Card buldun",
        time = "Kalan zaman: "
    }
}
UTK.searchinfo = {
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                            ...
                                            elseif not output then
                                                exports['mythic_notify']:SendAlert("error", "Hacker laptopuna sahip değilsin!")
                                            end
                                        end, "laptop_h")
                                    end
                                end
                                ...
                                if dst <= 4 then
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ...
                                        if output then
                                            ...
                                            self:IdCard()
                                        elseif not output then
                                            exports["mythic_notify"]:SendAlert("error", "ID Cardın yok!")
                                        end
                                        end, "id_card")
                                    end
                                end
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ESX.TriggerServerCallback("utk_oh:checkItem", function(output)
                                            if output then
                                                ...
                                                self:Plant()
                                            elseif not output then
                                                exports['mythic_notify']:SendAlert("error", "Thermal chargın yok!")
                                            end
                                        end, "thermal_charge")
                                    end
                                end
...
function UTK:Lockpick()
    ...
    exports['progressBars']:startUI(6000, self.text.lockpick..": Adım 1")
    Citizen.Wait(6500)
    exports['mythic_notify']:SendAlert("success", "Adım 1 "..self.text.stage)
    SetEntityCoords(ped, loc.x, loc.y, loc.z, 1, 0, 0, 1)
    SetEntityHeading(ped, loc.h)
    TaskPlayAnim(ped, animDict, "a_uncuff", 8.0, 8.0, 6000, 1, 1, 0, 0, 0)
    exports['progressBars']:startUI(6000, self.text.lockpick..": Adım 2")
    Citizen.Wait(6500)
    exports['mythic_notify']:SendAlert("success", "Adım 2 "..self.text.stage)
...
function UTK:Hack()
    ...
    Citizen.Wait(2000)
    Brute()
    exports["mythic_notify"]:SendAlert("success", "My Computer klasöründeki HackConnect.exe dosyasını aç")
        ...
        if HasScaleformMovieLoaded(scaleform) and UsingComputer then
            ...
            if GetScaleformMovieFunctionReturnBool(ClickReturn) then
                ...
                if program == 82 and not Hacking then
                    ...
                    PopScaleformMovieFunctionVoid()
                    Hacking = true
                    exports["mythic_notify"]:SendAlert("success", "IP Adresini bul...")
                elseif program == 83 and not Hacking and Ipfinished then
                    ...
                    Hacking = true
                    exports["mythic_notify"]:SendAlert("success", "Şifreyi bul...")
                ...
                elseif Hacking and program == 84 then
                    ...
                    Hacking = false
                    Ipfinished = true
                    exports["mythic_notify"]:SendAlert("success", "BruteForce.exe dosyasını aç")
                ...
                elseif Hacking and program == 86 then
                    ...
                    elseif UTK.hackmethod == 2 then
                        ...
                        UsingComputer = false
                        Ipfinished = false
                        Process(25000, "Sistem hackleniyor...")
...
AddEventHandler("utk_oh:vaulttimer", function(method)
    if method == 1 then
        exports['mythic_notify']:SendAlert("error", "Zehirli gaz gelmeden önce 90 saniyen var.")
        UTK.starttimer = true
        UTK.vaulttime = 90
        Citizen.CreateThread(function()
            while true do
        ...
        ESX.TriggerServerCallback("utk_oh:gettotalcash", function(result)
            ...
            exports["mythic_notify"]:SendAlert("success", "Çaldığın para miktarı:"..text)
        end)
    else
        exports['mythic_notify']:SendAlert("error", "Zehirli gaz gelmeden önce 90 saniyen var.")
        UTK.starttimer = true
...
AddEventHandler("utk_oh:gas_c", function()
    Citizen.CreateThread(function()
        UTK.begingas = true
        exports["mythic_notify"]:SendAlert("error", "Kasanın kapısı 60 saniye içerisinde otomatik olarak kapanacaktır!")
        while true do
            Citizen.Wait(1)
            if UTK.begingas then
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if UTK.begingas then
                ...
                UTK.stage2break = true
                UTK.stage4break = true
                exports["mythic_notify"]:SendAlert("error", "Kasa kapısı kapanıyor!")
...
AddEventHandler("utk_oh:policenotify", function(toggle)
    ...
    if player.job.name == "police" or player.job.name == "sheriff" then
        if toggle == 1 then
            exports["mythic_notify"]:SendAlert("infrom", "Alarm tetiklendi, polisler yolda!", 10000, {["background-color"] = "#CD472A", ["color"] = "#ffffff"})
