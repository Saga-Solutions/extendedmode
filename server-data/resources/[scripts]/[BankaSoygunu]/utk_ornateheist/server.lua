...
local mincash = 50000 -- minimum amount of cash a pile holds
local maxcash = 55000 -- maximum amount of cash a pile can hold
    ...
    if not info.locked then
        if (os.time() - cooldown) > lastrobbed then
            ...
            if copcount >= mincops then
                ...
                cb("Şehirde en az "..mincops.." polis olmalı!")
            end
        else
            cb(math.floor((cooldown - (os.time() - lastrobbed)) / 60)..":"..math.fmod((cooldown - (os.time() - lastrobbed)), 60).." zaman sonra bankayı soyabilirsin!")
        end
    else
        cb("Banka zaten soyuluyor!")
    end
end)
